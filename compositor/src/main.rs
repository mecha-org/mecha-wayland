...
use protocols::wl_surface::{SurfaceCommitted, WlSurfaceState};
...
struct Compositor {
    ...
    shm: WlShmState,
    surfaces: WlSurfaceState,
    client_window: ClientWindow,
}

fn blit(compositor: &mut Compositor, ev: &SurfaceCommitted) {
    // Collect what we need from the surface so we release its borrow before
    // accessing other Compositor fields.
    let (buf_id, prev_buf_id, frame_callbacks) = {
        ...
        let callbacks = surface.current.frame_callbacks.drain(..).collect::<Vec<_>>();
        (buf_id, prev_id, callbacks)
    };
    ...
    let dst_ptr = unsafe {
        libc::mmap(std::ptr::null_mut(), dst_size, libc::PROT_WRITE, libc::MAP_SHARED, prime_fd, 0)
    };
    ...
    unsafe { libc::munmap(dst_ptr as *mut _, dst_size) };
