            ...
            match ev {
                ...
                WlKeyboardEvent::Enter {
                    ...
                    if let Some(surf) = &compositor.seat.pointer_state.focused_surface {
                        for kb in &state.client_keyboards {
                            if kb.proxy.is_same_connection(&surf.proxy) {
                                kb.enter(serial, surf, keys);
                                state.focused_surface = Some(surf.clone());
                                state.focused_client = Some(kb.clone());
                ...
                WlKeyboardEvent::Leave {
                    ...
                    if let Some(surf) = &state.focused_surface
                        && let Some(kb) = &state.focused_client
                    {
                        kb.leave(serial, surf);
                        state.focused_surface = None;
                        state.focused_client = None;
                ...
                WlKeyboardEvent::Key {
                    ...
                    if let Some(kb) = &state.focused_client {
