...
impl SurfaceBackend for DmaBuf {
    fn allocate(renderer: &Renderer, width: u32, height: u32) -> Result<Self> {
        ...
        let gbm_bo = renderer._gbm_device
        ...
        let prime_fd    = gbm_bo.fd().map_err(|e| anyhow::anyhow!("gbm_bo_get_fd: {e}"))?;
        ...
        let import_attribs: [i32; 17] = [
            ...
            EGL_DMA_BUF_PLANE0_FD_EXT,          prime_fd.as_raw_fd(),
        ...
        Ok(Self { _gbm_bo: gbm_bo, prime_fd, egl_image, stride, modifier, rbo, fbo })
    }
    ...
    fn destroy(self, renderer: &Renderer) {
        unsafe {
            ...
            (renderer.fn_destroy_image)(
                renderer.display.as_ptr() as *const c_void,
                self.egl_image,
            );
        }
